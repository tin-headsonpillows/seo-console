---
version: alpha
name: SEO Console Design System
description: A clean, SEO-software-first interface anchored on white canvas with black primary CTAs and geometric display typography. The system reads as friendly modern SaaS — generous whitespace, soft-rounded cards (~12px), product UI fragments shown directly inside cards, and a dark footer that visually closes long-scroll pages. Brand voltage comes from the display headline and from product UI artifacts shown in-card rather than from accent colors.
...
SEO Console's marketing surface is a clean, friendly modern-SaaS interface — white canvas (`{colors.canvas}` — #ffffff) with black primary CTAs (`{colors.primary}` — #111111), custom **Manrope** display typography, and `{colors.surface-card}` (#f5f5f5) light-gray cards holding product UI fragments. The system reads as confidently engineered without trying to impress — every band has clear hierarchy, generous whitespace, and a single primary action.

Type voice splits cleanly into two roles: **Manrope** (the brand's custom geometric display face — used for h1, h2, h3, and hero headlines) and **Inter** (used for everything else — body, buttons, nav, captions). Manrope uses weight 600 with negative letter-spacing (-0.5px to -2px depending on size) — it feels modern, slightly condensed, distinctly SEO Console.

Component voltage comes from **product UI fragments shown directly inside cards** — keyword and ranking widgets, SEO audit forms, automation diagrams, integration tiles. SEO Console doesn't paint marketing illustrations of the product; it shows the actual product chrome at small scale embedded in the marketing flow.
...
- Custom `Manrope` display typeface for headlines (substituted with Inter weight 600 here). Negative letter-spacing on display sizes — geometric, precise, slightly condensed.
- Light-gray card surfaces (`{colors.surface-card}` — #f5f5f5) for feature cards, customer-proofs, and pricing tiers (non-featured). The featured pricing tier flips to `{colors.surface-dark}` (the only dark card on light pages).
- Product UI fragments embedded directly in cards — SEO Console shows real schedule pickers, keyword and ranking widgets, integration grids inside its marketing cards. Brand voltage from real product chrome at small scale.
...
- Avatars are circular (`{rounded.full}`), 36px diameter, used in customer-proof rows and team-listing surfaces.
...
- **Brand Accent** (`{colors.brand-accent}` — #3b82f6): Used sparely on inline links and on a small badge / "Customer story" highlight. SEO Console is a near-monochrome brand — the blue appears rarely.
...
- **Surface Card** (`{colors.surface-card}` — #f5f5f5): Feature cards, customer-proof cards, badge pills, default avatar fills.
...
### Semantic
- **Success** (`{colors.success}` — #10b981): Run auditation states, success badges in product UI.
...
### Font Family
The system runs **Manrope** for display + system wordmark and **Inter** for everything else. Manrope is SEO Console's geometric display typeface — slightly condensed, weight 600, negative letter-spacing. Inter handles body, buttons, navigation, captions, and tabular code blocks. The fallback stack walks `-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif` for both families.

The split is functional:
- Manrope (display, 600 weight, -0.5 to -2px tracking) — h1, h2, h3
...
|---|---|---|---|---|---|
| `{typography.display-xl}` | 64px | 600 | 1.05 | -2px | Homepage h1 ("The better way to monitor and improve your search performance") — Manrope |
| `{typography.display-lg}` | 48px | 600 | 1.1 | -1.5px | Section heads ("Your all-purpose SEO console") — Manrope |
| `{typography.display-md}` | 36px | 600 | 1.15 | -1px | Sub-section heads, card titles — Manrope |
| `{typography.display-sm}` | 28px | 600 | 1.2 | -0.5px | CTA-band heads, pricing tier prices — Manrope |
...
### Principles
Manrope is the brand voice — every display headline uses it. Inter handles the supporting type. The boundary is strict: never put body copy in Manrope, never put a display headline in Inter. Manrope without negative letter-spacing reads as off-brand — the -0.5 to -2px tracking is part of the voice.

Display weight stays at 600 across all sizes — never 700, never 500. The middle weight is what makes Manrope feel modern and confident without becoming bombastic.

### Note on Font Substitutes
If Manrope is unavailable, **Inter** at weight 600 with -0.04em letter-spacing is a usable approximation. The geometric character of Manrope differs from Inter's humanist forms, but the substitution preserves the weight + tracking signature. **Manrope** at weight 700 is another close alternative.
...
- **Card internal padding:** `{spacing.xl}` (32px) for feature cards and pricing tier cards; `{spacing.lg}` (24px) for customer-proof and product-mockup cards.
...
SEO Console uses generous but not excessive whitespace — section padding sits at 96px (modern-SaaS standard), and card internal padding stays at 32px. The rhythm is calibrated for fast scanning: every band has a single h1 + h2 + supporting cards, never densely packed lists. The result reads as confident-not-shouting.
...
| Card surface | `{colors.surface-card}` background — no shadow | Feature cards, customer-proofs |
...
- Avatar circles in customer-proof sections sometimes carry pastel fill colors (`{colors.badge-orange}`, `{colors.badge-pink}`, etc.) — adds a small chromatic flourish without breaking the monochrome brand voice.
...
| `{rounded.lg}` | 12px | Content cards (feature cards, customer-proof cards, pricing tier cards) |
...
Avatar photos use `{rounded.full}` (perfect circles) at 36px or 40px. Product UI fragments inside marketing cards retain their native chrome (which often has its own internal radii — e.g., keyword grid cells, button rows). Hero illustration zones use 16:9 or 4:3 ratios with `{rounded.xl}` corners.

## Components

### Top Navigation

**`top-nav`** — White nav bar pinned to the top of every page. 64px tall, `{colors.canvas}` background. Carries the SEO Console wordmark + logo at left (the lowercase "SEO Console" with the system mark), primary horizontal menu (Product, Solutions, Resources, Pricing, Agencies) center, right-side cluster with "Log in" text-link, "Start free" `{component.button-primary}`, and a sometimes-visible language selector. Menu items in `{typography.nav-link}` (Inter 14px / 500).

**`nav-pill-group`** — A small pill-radius wrapper around 2-3 sub-nav segments (e.g., the product-mode switcher between "Projects" / "Pro" / "Agencies"). Background `{colors.surface-soft}` with internal padding 6px, rounded `{rounded.pill}`. Active segment renders as a white-canvas pill with a subtle drop shadow inside the wrapper. The pill-in-pill treatment is one of SEO Console's signature interactive components.
...
**`button-text-link`** — Inline text button, no background. Used for "Log in" in the top nav and inline CTA links inside cards.
...
**`hero-app-mockup-card`** — A larger product-UI mockup card showing the actual SEO Console SEO dashboard widget with keyword grid, ranking positions, and a primary "Run audit" button inside. Background `{colors.canvas}`, 1px hairline border, rounded `{rounded.xl}` (16px), subtle drop shadow. Used as the hero's right-side artifact.

**`feature-card`** — Used in 3-up feature grids ("With us, SEO workflow management is easy"). Background `{colors.surface-card}` (#f5f5f5), rounded `{rounded.lg}` (12px), internal padding `{spacing.xl}` (32px). Carries a small icon at top, an `{typography.title-md}` headline, and a body description in `{typography.body-md}`.
...
**`product-mockup-card`** — A card showing actual SEO Console product UI fragments (SEO workflow editor, keyword grid, integration grid, automation flow). Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.lg}` (24px). The product UI inside has its own internal chrome — these cards display the product, they don't decorate around it.

**`customer-proof-card`** — Used in customer-proof grids. Background `{colors.surface-card}`, rounded `{rounded.lg}`, padding `{spacing.lg}` (24px). Top row carries a `{component.avatar-circle}` + name + role; below sits the customer-proof quote in `{typography.body-md}`.
...
**`pricing-tier-card-featured`** — The featured tier (typically "Pro"). Background flips to `{colors.surface-dark}` (#101010), text inverts to `{colors.on-dark}`. The dark surface IS the featured-tier signal — no accent border, no badge, no scale shift.
...
**`rating-stars`** — Inline star rating in `{colors.badge-orange}` (#fb923c). Used near customer-proof avatars to display a 5-star satisfaction score.
...
**`cta-band-light`** — A pre-footer "Clearer, faster SEO decisions" CTA card. Background `{colors.surface-card}`, rounded `{rounded.lg}`, padding `{spacing.xxl}` (48px). Carries an h2 in `{typography.display-sm}`, a sub-line, and a `{component.button-primary}` centered.

**`footer`** — Dark navy footer that closes every page. Background `{colors.surface-dark}` (#101010), text `{colors.on-dark-soft}`. 4-column link list at desktop covering Projects / Audits / Reports / Resources. Vertical padding 64px. The SEO Console wordmark sits at the top-left in `{colors.on-dark}`. The footer is the only dark surface on every page — the deliberate inversion visually closes the page.

## Do's and Don'ts

### Do
- Reserve `{colors.primary}` (#111111) for primary CTAs and h1/h2 type. SEO Console's button is near-black, not blue.
- Use Manrope for every display headline. Pair with Inter body. Never blur the boundary.
- Apply negative letter-spacing on display sizes (-0.5 to -2px). Manrope without it reads as off-brand.
...
- Don't bold display weight beyond 600. Manrope at 700 reads as bombastic.
- Don't use rounded radius beyond `{rounded.xl}` (16px) on cards. Larger radii read as consumer-app, not professional SEO software.
...
- Don't repeat the same surface mode in two consecutive bands. SEO Console's pacing alternates white → light-gray → white → product-mockup-card → white → dark-footer.
...
- Avatar + customer-proof card layouts stay grid-aligned at every breakpoint.
...
- Hero app-mockup card scales proportionally on mobile — the keyword grid stays legible.
...
5. Display headlines stay Manrope 600 with negative letter-spacing. Body stays Inter 400. The trinity does not blur.
6. The dark footer is the only dark surface on most pages. Don't add other dark cards casually.
7. When in doubt about emphasis: bigger Manrope before bolder Manrope.

## Known Gaps

- The dembrandt frequency analyzer captured `Buttons: 0 variants` — SEO Console renders most CTAs as styled `<a>` link elements rather than `<button>` tags, which dembrandt's button selector doesn't capture. Button styles are documented from screenshot ground-truth + standard Manrope / Inter baselines.
- Manrope is licensed to SEO Console and not available as a public web font; substitutes are documented in the typography section.
...
- The actual SEO dashboard widget surface (seo-console/{username}) is the product, not a marketing surface; its spec is out of scope.
- Avatar photos in customer-proof sections sometimes carry pastel circular fills with initials instead of photographs; both treatments coexist on the same page.
