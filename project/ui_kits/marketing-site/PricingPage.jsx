function PricingPage({ onNavigate }) {
  const { PricingTierCard, CtaBandLight } = window.SEOConsoleDesignSystem_8b9179;
  ...
  return (
    <div>
      <div style={{ ...section, textAlign: "center", display: "flex", flexDirection: "column", gap: "var(--space-md)", alignItems: "center" }}>
        <h1 style={{ margin: 0, fontFamily: "var(--display-lg-family)", fontSize: "var(--display-lg-size)", fontWeight: 600, letterSpacing: "var(--display-lg-tracking)", lineHeight: "var(--display-lg-leading)", color: "var(--text-ink)" }}>
      ...
      <div style={{ ...section, paddingTop: 0, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "var(--gutter-card)" }}>
        {TIERS.map((t) => (
          <PricingTierCard key={t.name} {...t} onCtaClick={() => onNavigate("login")} />
        ))}
      </div>
