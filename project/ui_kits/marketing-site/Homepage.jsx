function Homepage({ onNavigate }) {
  const { HeroBand, Wordmark } = window.SEOConsoleDesignSystem_8b9179;
  const HeroAppMockupCard = window.SEOConsoleDesignSystem_8b9179.HeroAppMockupCard;
  const { FeatureCard, ProductMockupCard, CustomerProofCard, CtaBandLight, Avatar, Button, Icon } = window.SEOConsoleDesignSystem_8b9179;
  const section = { maxWidth: "var(--content-max)", margin: "0 auto", padding: "var(--pad-band) var(--space-xl)", boxSizing: "border-box" };
  ...
  return (
    <div>
      <HeroBand
        ...
        actions={
          <React.Fragment>
            <Button onClick={() => onNavigate("login")}>Start free</Button>
            <Button variant="secondary" iconLeft={<Icon name="play" size={16} />}>Watch demo</Button>
          </React.Fragment>
        }
        mockup={<HeroAppMockupCard><KeywordGridFragment /></HeroAppMockupCard>}
      />

      <div style={{ background: "var(--surface-card)" }}>
        <div style={{ ...section, display: "flex", flexDirection: "column", gap: "var(--space-xxl)" }}>
          <h2 style={{ margin: 0, fontFamily: "var(--display-lg-family)", fontSize: "var(--display-lg-size)", fontWeight: 600, letterSpacing: "var(--display-lg-tracking)", lineHeight: "var(--display-lg-leading)", color: "var(--text-ink)", textAlign: "center" }}>
      ...
      <div style={section}>
        <div style={{ display: "grid", gridTemplateColumns: "5fr 7fr", gap: "var(--space-xxl)", alignItems: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-md)" }}>
            <h3 style={{ margin: 0, fontFamily: "var(--display-md-family)", fontSize: "var(--display-md-size)", fontWeight: 600, letterSpacing: "var(--display-md-tracking)", lineHeight: "var(--display-md-leading)", color: "var(--text-ink)" }}>
          ...
          <ProductMockupCard><AutomationFragment /></ProductMockupCard>
        </div>
      </div>

      <div style={{ background: "var(--surface-card)" }}>
        <div style={{ ...section, display: "flex", flexDirection: "column", gap: "var(--space-xxl)" }}>
          <h2 style={{ margin: 0, fontFamily: "var(--display-lg-family)", fontSize: "var(--display-lg-size)", fontWeight: 600, letterSpacing: "var(--display-lg-tracking)", color: "var(--text-ink)", textAlign: "center" }}>
      ...
      <div style={{ ...section, paddingTop: "var(--space-xxl)", paddingBottom: "var(--space-xxl)" }}>
        <CtaBandLight heading="Clearer, faster SEO decisions" subline="Start free — no card required." ctaLabel="Start free" onCtaClick={() => onNavigate("login")} />
      </div>
    </div>
  );
}
